        ...
        <div>
            ...
            <div className="relative shrink-0 h-48">
                <div className="h-full relative rounded-xl overflow-hidden border border-primary/20 bg-primary/5 focus-within:border-primary transition-all duration-200 shadow-sm">
                    <textarea
                        className="w-full h-full p-4 bg-transparent border-none focus:ring-0 text-base resize-none placeholder:text-slate-600 font-sans text-slate-100 outline-none"
                        maxLength={MAX_CHARS}
                        placeholder="Describe environmental findings..."
                        value={value}
                        onChange={(e) => onChange(e.target.value)}
                    ></textarea>
                    <div className="absolute bottom-3 right-3 bg-background/80 px-2 py-1 rounded-lg border border-primary/10 backdrop-blur-sm">
