...
const EmissionModal: React.FC<EmissionModalProps> = ({ onClose, onSave }) => {
  const [source, setSource] = useState('New Point Source');
      ...
      <div className="bg-[#1a242d] w-full max-w-sm rounded-xl shadow-2xl overflow-hidden border border-white/5 animate-in fade-in zoom-in duration-300">
        ...
        <div className="p-6 pb-4 border-b border-white/5">
          <div className="flex items-center gap-3 mb-1">
            <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
              <span className="material-icons text-primary text-xl">add_chart</span>
            </div>
            <h2 className="text-xl font-bold tracking-tight">Add Emission Data</h2>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input 
              className="w-full bg-black/20 border-2 border-white/10 rounded-lg px-4 py-2 text-sm font-medium focus:border-primary focus:ring-0 transition-all outline-none text-slate-100"
              value={source}
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <input 
                className="w-full bg-black/20 border-2 border-white/10 rounded-lg px-4 py-4 text-2xl font-bold focus:border-primary focus:ring-0 transition-all outline-none text-slate-100"
                type="number"
                step="0.01"
                placeholder="0.00"
                value={concentration}
